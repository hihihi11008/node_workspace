...
//http 메카니즘이다. 만일응답을 안하면?? 클라이언트는 무한정 서버의  응답을 기다리므로 무한대기상태에 빠진다.
//서버객체를 생성 
...
var server=http.createServer(function(request,response){
    ...
    //HTTP 상태코드중 200은 정상처리를 의미 (즉 서버에서 클라이언트의 요청을 정상적으로 처리했다는 상태코드 )
    //누가정했나? W3C 표준에 의해 정해진 것임 
    //참고 500은 심각한 서버의 에러, 404는 요청한 자원을 찾을 수 없을때 
    response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"}); // 편지봉투구성하기! 
    // response.writeHead(200,{"Content-Type":"image/jpg"}); // 편지봉투구성하기! 
    var tag="";
    /*
    tag+="<html>";
    tag+="<head>";
    ...
    */
    ...
    // fs.readFile("../images/gabi/5.jpg", function(error, data){
    //     response.end(data);
    // });
...
왜?? 하나의 os 내에서 수많은 네트워크 프로그램들간 엉키지 않기위해 
ex)오라클 1521 포트, mysql 3306 포트
*/
server.listen(7777, function(){
    console.log("Server is running at port at 7777 port...");
});