...
var con; //접속 성공시 그 정보를 보유한 객체, 이 객체가 있어야 접속된 상태에서 쿼리문을 수행할 수 있따 .
//서버객체 생성
var server=http.createServer(function(request,response){
    // response.end("this text is from my server");
    //클라이언트가 원하는 것이 무엇인지를 구분하기 위한 url 분석 
    console.log("클라이언트의 요청 url",request.url); //한줄로 되어있어 구분을 못한다 ... 
    ...
    // 직관성이 높아진다. 즉 다루기 쉬워진다. (배열보다 낫다^_^)
    ...
    if(parseObj.pathname=="/member/registForm"){
        fs.readFile("./registForm.html", "utf-8", function(error,data){
            if(error){
                ...
                response.end(data); //html 파일을 읽어들인 결과문자열을 클라이언트에 응답정보로  저장 
            }
        });
