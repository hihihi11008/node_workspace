...
GET : 가져오다 ( 서버로부터 컨텐츠를 가져올때 사용되는 요청방식)
        ...
        또한 편지봉투에 데이터를 전송하면 편지지에 비해 전송할 수 있는 데이터량에한계가 있다 
    주용도) 다른 URL 로 링크를 통해 파라미터 전송할떄 

POST : 보내다는 의미 (클라이언트가 서버에 데이터를 전송할때 사용하는 방식)
            HTTP 프로토콜의 body를 통해 데이터를 전송하기 때문에 편지지에 데이터를 실어서 보내는것과 같다, 
            봉투안에 담아서 보내기 때문에 데이터 노출되지 않는다 .
            특징) 보낼 수 있는 데이터량에 한계가 없다 
            용도) 노출되면 위험한 데이터를 전송시 사용(로그인요청, 회원가입 등 )
                    파일(사진,동영상 등 )등을 서버에 전송시

            post 방식은 form태그를 이용해서 요청할 수 있따 
*/
...
var server = http.createServer(function(request, response){
    ...
    if(request.method=="GET"){
        ...
        var urlJson = url.parse(request.url,true); // true매개변수를 넘기면 파라미터 정보를 json으로 변환 
        // console.log("URL분석 결과",urlJson);
        
        var paramJson=urlJson.query;
        console.log("ID : "+paramJson.id);
        console.log("PASSWORD : "+paramJson.pass);
        ...
        request.on("data", function(param){
            ...
            console.log("PASSWORD : "+postParam.pass);
        });
    }
});
