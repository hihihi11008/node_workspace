...
var con; // mysql 접속 정보를 가진 객체, 이객체로 sql 문을 수행할 수 있다 

//서버 객체 생성 
var server = http.createServer(function(request,response){

    //클라이언트가 원하는 요청을 처리하려면, URL 분석을 하여 URI(전체경로에서-도메인을뺸주소)추출해서 조건을 따져보자
    var urlJson = url.parse(request.url, true); //분석결과를 json으로 반환해줌
    ...
    if(urlJson.pathname=="/"){
        fs.readFile("./index.html","utf-8",function(error,data){
            if(error){
                console.log("index.html 읽기 실패", error); 
                    ...
                    //200이란? HTTP상태코드, 404 존재하지않는 자원 , 500 서버의 심각한에러
        ...
        //브라우저에서 전송된 파라미터를 먼저 받아야한다 
        //get 방식의 파라미터 받기 
        //회원정보는 member2 테이블에 넣고 
        var sql = "insert into member2(uid,password,uname,phone,receive,addr,memo,email)";
        sql+=" values(?,?,?,?,?,?,?,?)";
        ...
        con.query(sql,[
            ...
            param.addr,
            param.memo,
            param.email],function(error, result, fields){
                if(error){
        ...
        con.query(sql, function(error, record, fields){
            ...
            //(uid,password,uname,phone,receive,addr,memo,email
            //응답 정보를 테이블로 구성하자
            ...
            for(var i =0; i<record.length;i++){
                var member=record[i];//각각의 json 을 끄집어 내자
                tag+="<tr>";
        ...
        con.query(sql, function(error, record, fields){
            if(error){
                ...
                // console.log("5번회원의 정보 : ", record);
                var obj = record[0]; //0번째에 들어 있는 json 추출 
        ...
        fs.readFile("./test.ejs","utf-8", function(error,data){
            ...
            //서버에서 실행을 한 후, 그 결과를 보내야 한다 (jsp, php, asp 의 원리)
            response.end(ejs.render(data,{
                fruit:f1
            }));
        });
